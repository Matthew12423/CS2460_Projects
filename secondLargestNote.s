...
main:
	...
	li $t0, 0	#(I'd delete this and add a line for addi $sp, $sp, -20)
	la $t1, buffer				#load address of buffer
	
	#(I'd replace the next two lines with sw $t0, 0($sp) and sw $ra, 4($sp).)
	li $t8, 0					#max index
...
Loop:
	...
	#(I'd add in lw $t4, ($t2) and lw $t5, ($t2))
...
updateMax:						#the number we read is the largest so far
	...
	#(I'd replace the next two lines with lw $t0, 0($sp) and lw $ra, 4($sp).)
...
updateSecondLargest:			#update second largest
	...
	#(I'd change these two lines to sw $t7, ($t3) and sw $t6, 4($3))
	move $t7, $t3
	move $t6, $t0
